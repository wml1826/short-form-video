...
void MyOpenGLWidget::paintGL()
{
    ...
    if ( !m_image.isNull())
    {
        ...
        // 将位置转换为相对于窗口大小的比例（转换为OpenGL能识别的{-1，1}的坐标系）
        ...
        /// 指定第一个顶点的纹理坐标和顶点坐标。
        /// glTexCoord2f函数设置纹理坐标，(0, 1)表示纹理坐标的左上角（通常为(0, 0)到(1, 1)之间）。
        /// glVertex2d函数设置顶点坐标，(left, bottom)表示矩形的左下角顶点的坐标。
        /// glVertex2d函数设置的点为比例,
        /// 左下正常是(-1,-1), 不过图片居中调整后根据比例换算为( left, bottom )
        /// 以此类推 , 其他点类似（因为 OpenGL 是从图片的底部开始读取的，所以图片的底部就被读取到了 OpenGL 纹理坐标系的顶部。）
...
void MyOpenGLWidget::resizeGL(int w, int h)
{
    //按照固定缩放比 改变
    qDebug() << __func__;
    //设置视口大小（从左下角的{0，0}开始，一直延申到高h，宽w的地方）
    glViewport(0, 0, w, h);
    // 输出窗口大小信息
    qDebug() << QSize(w,h);
...
{
    //获取图像的大小，与窗口大小做比对，进行比例缩放
    //（利用宽高比，然后图像的宽高比大于窗口，就贴满窗口的宽，等比例缩放图像的高；反之则贴满图像的高，等比例贴满图像的宽）
